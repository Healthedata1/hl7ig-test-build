...
SearchParameter works too but can't be sorted by Type and no title element
-->
...
{% for i in my_array offset:1 %}
  {%- for r_hash in site.data.resources -%}
      {%- assign r_type = r_hash[0] | split: '/' | first -%}
      {%- assign r_id = r_hash[0] | split: '/' | last -%}
      {%- assign r = r_hash[1] -%}
      {%- if r.name == i and r_type == include.type %}
        ...
        {%- else %}
          {% assign r_id = r_hash[0] | split: '/' | last %}
          {% assign words =  r_id | split: '-' %}
          {% capture titlecase %}
          {% for word in words %}
            {{ word | capitalize }}{% unless forloop.last %} {% endunless %}
          {% endfor %}
          {% endcapture %}
          <li><a href="{{r.path}}" >{{ titlecase | replace_first: 'Us', 'US'}}</a></li>
        {% endif -%}
    {%- endif -%}
  {%- endfor -%}
{% endfor %}
