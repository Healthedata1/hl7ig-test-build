...
<!-- ================================================================
 The following scopes tables are generated by the liquid scrips below.
 The source table is input/data/profile_metadata.csv where edits to the table contents are made
 ====================================================================== -->

{% assign rows = site.data.profile_metadata -%}
{%- assign shall_scopes = '' -%}
{%- assign should_scopes = '' -%}
{%- assign shall_granular_scopes = '' -%}
{%- assign should_granular_scopes = '' -%}
{%- for item in rows -%}
  {%- if item.resource_scope_conf == "SHALL" -%}
    ...
    {%- assign shall_scopes =  shall_scopes | append: scope -%}
      {% for i in (1..6) %}
        ...
        {%- elsif item[cat] and item[cat_conf] == "SHOULD" -%}
          {%- assign should_granular_scopes =  should_granular_scopes | append: scope -%}
        {%- endif -%}
      {%- endfor -%}
  ...
  {%- endif -%}
{%- endfor -%}
{% assign all_granular_scopes = shall_granular_scopes | append: "," | append: should_granular_scopes %}
...
<tbody>
{% assign scope_array = shall_granular_scopes | split: "," | uniq | sort -%}
...
<tbody>
{% assign scope_array = should_granular_scopes | split: "," | uniq | sort -%}
{% for granular_scope in scope_array -%}
<tr>
<td>{{granular_scope | split: '.' | first }}</td>
<td><code>{{ granular_scope | prepend: '<patient|user|system>/' }}</code></td>
